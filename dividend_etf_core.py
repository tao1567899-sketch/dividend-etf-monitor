  ...
  def tushare_call(api_name: str, params: dict, fields: str = "", config: dict =
   None) -> pd.DataFrame:         
      """                                                                       
      ...
      if result.get("code") != 0:                                               
          raise RuntimeError(f"Tushare 接口返回错误 [{api_name}]: 
  {result.get('msg')}")                                                         
  ...
  def is_trade_day(date_str: str, config: dict) -> bool:                        
      ...
      df = tushare_call(                                                        
          "trade_cal",                                                          
          params={"exchange": "SSE", "start_date": date_str, "end_date":
  date_str},                                                                    
  ...
  def screen_dividend_etfs(config: dict) -> pd.DataFrame:                       
      ...
      """                                                                       
      # 1. 获取全市场 ETF 基础信息（含 benchmark 用于去同质化）                 
      df = tushare_call(                                                        
          "fund_basic",           
          params={"market": "E", "status": "L"},                                
          fields="ts_code,name,fund_type,market,list_date,benchmark",           
          config=config,                                                        
      )                                                                         
      ...
      # 3. 上市时间过滤（≥6个月）
      cutoff = (datetime.today() - timedelta(days=MIN_LIST_MONTHS *             
  30)).strftime("%Y%m%d")                                                       
      ...
      daily_df = tushare_call(    
          "fund_daily",                                                         
          params={"ts_code": codes, "start_date": start_date, "end_date":
  end_date},                                                                    
      ...
      daily_df["amount"] = pd.to_numeric(daily_df["amount"],                    
  ...
  format="%Y%m%d")                                                              
      ...
      # 5. 去同质化：同一基准指数只保留成交额最高的一只                         
      df["benchmark"] = df["benchmark"].fillna("").str.strip()                  
      has_bm = df["benchmark"] != ""                                            
      df_dedup = (                
          df[has_bm]
          .sort_values("avg_amount", ascending=False)
          .drop_duplicates(subset=["benchmark"], keep="first")                  
      )
      df = pd.concat([df_dedup, df[~has_bm]], ignore_index=True)                
      logger.info(f"去同质化后红利ETF共 {len(df)} 只")                          
  ...
  adjust=False).mean()                                                          
      ...
      # 避免除零：avg_loss 为 0 时 RSI = 100（纯上涨）；avg_gain 为 0 时 RSI =  
  0（纯下跌）                     
  ...
  def calculate_ttm_yield(ts_code: str, current_price: float, div_df:           
  pd.DataFrame) -> float:         
      """                                                                       
      计算 TTM（近12个月）股息率。
      ...
      total_div = pd.to_numeric(code_div["div_cash"],
  ...
  MAX_POSITIONS = 10           # 最大持仓只数（买入+持有合计）
  ...
  30)).strftime("%Y%m%d")                                                       
      try:                        
          df = tushare_call(                                                    
              "fund_daily",       
              params={"ts_code": ts_code, "start_date": start_date, "end_date": 
  end_date},
          ...
          df["close"] = pd.to_numeric(df["close"], errors="coerce")             
          return                  
  df.dropna(subset=["close"]).sort_values("trade_date").reset_index(drop=True)  
  ...
  end_date},                                                                    
          ...
          df["div_cash"] = pd.to_numeric(df["div_cash"],
  errors="coerce").fillna(0)                                                    
          return df
      except Exception as e:                                                    
  ...
  def run_backtest(
      ...
      if len(df) < 100:                                                         
          return {"annualized_return": 0.0, "max_drawdown": 0.0, "total_trades":
   0, "win_rate": 0.0}                                                          
      ...
      # ─── 预计算周线 RSI 系列（避免 O(n²)）───                                
      df_for_rsi = df.copy()
      df_for_rsi["trade_date_dt"] = pd.to_datetime(df_for_rsi["trade_date"],    
  format="%Y%m%d")                                                              
      ...
      if len(weekly_close) >= 15: 
          ...
          rsi_map = {week_end: rsi_val for week_end, rsi_val in zip(rsi_index,
  rsi_series.values.tolist())}                                                  
      ...
      for i in range(len(df)):
          ...
          if position > 0:                                                      
              ...
              if sell_reason:                                                   
                  ...
                  capital = proceeds                                            
                  trades.append({"date": current_date, "profit_pct": profit_pct,
   "reason": sell_reason})                                                      
                  position = 0    
                  buy_price = 0.0                                               
  ...
  按100份整手买入                                                               
      ...
      if position > 0:                                                          
          ...
          trades.append({"date": str(df.iloc[-1]["trade_date"]), "profit_pct":
  profit_pct, "reason": "END"})                                                 
      ...
      win_rate = len(win_trades) / len(closed_trades) * 100 if closed_trades
  else 0.0                                                                      
  ...
  def main():
      ...
      for _, row in etf_list.iterrows():                                        
          ...
          try:
              ...
              # 7. 回测（仅买入标的计算回测）                                   
              backtest_result = {"annualized_return": 0.0, "max_drawdown": 0.0,
  "total_trades": 0, "win_rate": 0.0}                                           
              if signal == "BUY": 
                  backtest_div_df = fetch_8year_div(ts_code, config)            
                  backtest_result = run_backtest(ts_code, daily_df,             
  backtest_div_df)                                                              
      ...
      if len(buy_list) > 1:                                                     
          ...
          logger.info(f"多只买入信号，选择股息率最高：{buy_list[0]['ts_code']} 
  {buy_list[0]['yield_pct']:.2f}%")                                             
          hold_list.extend(buy_list[1:])
          buy_list = [buy_list[0]]                                              
                                  
      # 持仓上限：买入+持有合计不超过 MAX_POSITIONS，按质量排序保留             
      max_hold = MAX_POSITIONS - len(buy_list)
      if len(hold_list) > max_hold:                                             
          def _hold_rank(x):      
              # 有效股息率优先（越高越好），其次 RSI 越低越优先（越超卖越好）   
              return (0, -x["yield_pct"], x["rsi"]) if x["yield_pct"] > 0 else  
  (1, 0, x["rsi"])                                                              
          hold_list.sort(key=_hold_rank)                                        
          logger.info(f"持仓上限 {MAX_POSITIONS}，裁剪持有列表：{len(hold_list)}
   → {max_hold}")                                                               
          hold_list = hold_list[:max_hold]                                      
                                                                                
      # 8. 飞书推送                                                             
      date_display = datetime.today().strftime("%Y-%m-%d")
      report = format_report(date_display, buy_list, hold_list, sell_list,      
  total_count)                                                                  
      push_to_feishu(config["feishu_url"], report)
                                                                                
      logger.info(f"[DONE]                                                      
  完成：买入{len(buy_list)}只，持有{len(hold_list)}只，卖出{len(sell_list)}只") 
